          ...
          <div className="member-welcome-left">
            ...
            <div className="member-welcome-info">
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.35rem' }}>
                ...
                <span className={`status-pill status-${member?.status || 'active'}`}>{member?.status || 'active'}</span>
              </div>
              <h1>Welcome, {member?.user?.name || user?.name}!</h1>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.92rem' }}>
                Member ID: <span style={{ fontFamily: 'monospace', color: 'var(--text-secondary)' }}>{member?._id}</span>
              </p>
            </div>
          </div>

          <Button variant="secondary" size="sm" icon={Edit2} onClick={() => setEditProfileModalOpen(true)}>
            Edit Profile
          </Button>
        </Card>
        ...
        <div className="member-dashboard-grid">
          ...
          <Card className="glass-panel" padding="normal">
            <div className="card-title-header">
              <h3><CreditCard size={20} className="text-highlight" /> My Membership Plan</h3>
              {membership?.plan ? (
                <Badge variant="primary" size="sm">{membership.plan.name}</Badge>
              ) : (
                <Badge variant="outline" size="sm">No Plan</Badge>
              )}
            </div>
            ...
            {membership?.plan ? (
              <div>
                <div className="membership-plan-headline">
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <span style={{ fontSize: '1.8rem', fontFamily: 'var(--font-heading)', fontWeight: 900, color: '#ffffff' }}>
                      ${membership.plan.price}
                    </span>
                    <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}> / {membership.plan.duration} mo</span>
                  </div>
                </div>
                ...
                <div className="days-counter-box">
                  <div>
                    <span style={{ fontSize: '0.8rem', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700, display: 'block' }}>
                      Access Remaining
                    </span>
                    <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                      Valid until {membership.endDate ? new Date(membership.endDate).toLocaleDateString() : 'N/A'}
                    </span>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <span className={`days-count-num ${membership.isExpiringSoon ? 'expiring-soon' : ''}`}>
                      {membership.daysRemaining}
                    </span>
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', display: 'block' }}>Days Left</span>
                  </div>
                </div>
                ...
                <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: '1.25rem' }}>
                  ...
                  <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.92rem', color: 'var(--text-secondary)' }}>
              ...
              <div className="empty-state-box">
                ...
                <p style={{ fontSize: '0.88rem' }}>Please consult the IronForge front desk to activate a membership tier.</p>
              </div>
            )}
          </Card>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-xl)' }}>
            ...
            <Card className="glass-panel" padding="normal">
              <div className="card-title-header">
                <h3><Award size={20} className="text-highlight" /> My Assigned Coach</h3>
              </div>
              ...
              {trainer ? (
                <div className="trainer-assigned-box">
                  <div className="trainer-card-header-row">
                    ...
                    <div>
                      ...
                      <Badge variant="outline" size="sm">{trainer.specialization}</Badge>
                    </div>
                  </div>

                  <p style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', lineHeight: '1.5' }}>
                    {trainer.bio || 'Your assigned coach for personalized workout guidance and routine optimization.'}
                  </p>

                  <div style={{ background: 'var(--bg-surface-elevated)', padding: '0.75rem 1rem', borderRadius: 'var(--radius-md)', display: 'flex', flexDirection: 'column', gap: '0.35rem', fontSize: '0.85rem' }}>
                ...
                <div className="empty-state-box" style={{ padding: '2rem 1rem' }}>
                  <div className="empty-state-icon"><Award size={24} /></div>
                  <span className="empty-state-title" style={{ fontSize: '1rem' }}>No Personal Coach Assigned</span>
                  <p style={{ fontSize: '0.82rem' }}>You are currently on independent gym access. Request coach assignment from administration.</p>
                </div>
              )}
            </Card>
            ...
            <Card className="glass-panel" padding="normal">
              <div className="card-title-header">
                <h3><UserCheck size={20} className="text-highlight" /> Personal Details</h3>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.9rem' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '0.4rem' }}>
                  ...
                  <span style={{ textTransform: 'capitalize' }}>{member?.gender || 'Unspecified'}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '0.4rem' }}>
                  <span style={{ color: 'var(--text-muted)' }}>Emergency Contact:</span>
                  <span>
                    {member?.emergencyContact?.name
                      ? `${member.emergencyContact.name} (${member.emergencyContact.relation || 'Emergency'})`
                      : 'None'}
                  </span>
                </div>
              </div>
